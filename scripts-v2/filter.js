...
const all_speed = {"Slow":"Yavaş","Normal":"Normal","Fast":"Hızlı"}
const all_sanity = {"Late":"Geç","Average":"Ortalama","Early":"Erken","VeryEarly":"ÇokErken"}
let all_evidence = []
let all_ghosts = []
let all_maps = {}
let bpm_list = []
...
var user_settings = {"num_evidences":"3","cust_num_evidences":"3","cust_hunt_length":"3","ghost_modifier":2,"volume":50,"mute_timer_toggle":0,"mute_timer_countdown":0,"timer_count_up":0,"timer_split":1,"offset":0.0,"sound_type":0,"speed_logic_type":0,"bpm":0,"domo_side":0,"priority_sort":0,"map":"6 Tanglewood Drive","theme":"Varsayılan"}
...
function closeMenu(){
    ...
    if(mquery.matches){
        document.getElementById("menu").style.marginBottom = is_c ? "-695px" : "-645px";
...
function flashMode(){
    ...
    var mode_text = {"-1":"Özel","0":"Kıyamet III","1":"Cinnet","2":"Karabasan","3":"Profesyonel","3I":"Orta","3A":"Amatör"}[cur_evidence]
    document.getElementById("game_mode").innerHTML = `${mode_text}<span>(${parseInt(cur_evidence)} kanıt)</span>`.replace("-1",document.getElementById("cust_num_evidence").value)
...
function loadSettings(){
    ...
    try{
        ...
        user_settings = {"num_evidences":"3","cust_num_evidences":"3","cust_hunt_length":"3","ghost_modifier":2,"volume":50,"mute_timer_toggle":0,"mute_timer_countdown":0, "timer_count_up":0,"timer_split":1,"offset":0.0,"sound_type":0,"speed_logic_type":0,"bpm_type":0,"bpm":0,"domo_side":0,"priority_sort":0,"map":"6 Tanglewood Drive","theme":"Varsayılan"}
    }
...
function showCustom(){
    ...
    if(document.getElementById("num_evidence").value == "-1"){
        if(mquery.matches){
            document.getElementById("menu").style.height="765px";
            if(is_h){
                document.getElementById("menu").style.marginBottom = "-695px";
                $("#domovoi").removeClass("domovoi-custom")
                $("#domovoi").addClass("domovoi-menu-hidden")
            }
            else{
        ...
        if(mquery.matches){
            if(is_h){
                document.getElementById("menu").style.marginBottom = "-645px";
                $("#domovoi").removeClass("domovoi-custom")
                $("#domovoi").addClass("domovoi-menu-hidden")
            }
            else{
            ...
            document.getElementById("menu").style.height="710px";
        }
    }
}
