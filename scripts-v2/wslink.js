                ...
                if (incoming_state['action'].toUpperCase() == "POLL"){
                    ...
                    if(Object.keys(discord_user).length > 0){
                        if (hasSelected()){
                            ...
                            $("#reset").html("Başkalarını bekliyorum...")
                        }
                        else{
...
function send_empty_data_link(){
    if(hasDLLink){
        dlws.send(JSON.stringify({"action":"GHOSTDATA","ghost":`None|<i>Anlatımlarını ve davranışlarını görmek için bir hayalete tıklayın\n(Hayaletler arasında geçiş yapmak için ' ğ ' ve ' ü ' tuşlarını kullanın)<i>`}))
    }
}
